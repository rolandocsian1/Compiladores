...
OPERATORS = {'TOW','GAP','ERS','STINT','FUEL_DELTA','DEAD_HEAT','OUTLAP','UNDERCUT','OVERCUT','BOTH_TYRES','EITHER_TYRE','REVERSE_GRID','SETUP'}
LITERALS  = {'INT_LITERAL','FLOAT_LITERAL','STRING_LITERAL','CHAR_LITERAL'}
DELIMITERS= {'CORNER','APEX','GARAGE','READY','GRID','ENDGRID','STOP','ALSO','MARSHALL'}
...
def generate_token_report(token_list, error_list, source_file, output_path):
    ...
    stat_cards = ""
    colors = {
        'Palabra Reservada': '#e10600', 'Operador': '#ff8c00',
        'Literal': '#4fc3f7', 'Identificador': '#a5d6a7',
        'Delimitador': '#ce93d8', 'Otro': '#888'
    }
...
<head>
  ...
  {HTML_STYLE}
  <style>
    .cat-badge {{ display: inline-block; padding: 2px 8px; border-radius: 3px; font-size: 0.75rem; font-weight: bold; }}
    ...
    .filter-btn {{ padding: 6px 14px; border-radius: 4px; border: 1px solid #333; background: #1a1a1a; color: #ccc; cursor: pointer; font-size: 0.82rem; transition: all 0.2s; }}
    .filter-btn:hover, .filter-btn.active {{ background: #e10600; color: white; border-color: #e10600; }}
    .stats-row {{ display: flex; gap: 14px; flex-wrap: wrap; margin-bottom: 24px; }}
    .stat-card {{ background: #1a1a1a; border-radius: 6px; padding: 14px 20px; min-width: 130px; text-align: center; }}
...
<body>
  ...
  <table id="tokenTable">
    <thead><tr><th>Token</th><th>Lexema</th><th>Categoría</th><th>Línea</th><th>Columna</th></tr></thead>
    <tbody>{rows}</tbody>
  </table>
  {error_section}
  <script>
    let currentCat = '';
    function filterCat(cat, btn) {{ currentCat = cat; document.querySelectorAll('.filter-btn').forEach(b => b.classList.remove('active')); btn.classList.add('active'); applyFilters(); }}
    function filterTable() {{ applyFilters(); }}
    function applyFilters() {{ const search = document.getElementById('searchBox').value.toLowerCase(); document.querySelectorAll('#tokenTable tbody tr').forEach(row => {{ const cat = row.getAttribute('data-cat') || ''; const text = row.innerText.toLowerCase(); row.style.display = (!currentCat || cat === currentCat) && (!search || text.includes(search)) ? '' : 'none'; }}); }}
  </script>
...
def generate_symbol_table_report(symbol_table, source_file, output_path):
    ...
    html = f"""<!DOCTYPE html>
<html lang="es"><head><meta charset="UTF-8"><title>PitCode - Tabla de Símbolos</title>{HTML_STYLE}</head>
<body>
  <h1>PitCode — Tabla de Símbolos</h1>
  <div class="summary">Archivo: <strong>{source_file}</strong> | Símbolos: <strong>{len(symbols)}</strong></div>
  <h2>Símbolos <span class="badge ok">{len(symbols)}</span></h2>
  <table><thead><tr><th>Nombre</th><th>Tipo</th><th>Rol</th><th>Ámbito</th><th>Línea</th><th>Columna</th></tr></thead>
...
def generate_error_report(lex_errors, syn_errors, source_file, output_path):
    def make_rows(errors, tag):
        rows = ""
        for e in errors:
            rows += f"""<tr><td class="tag-error">{tag}</td><td class="tag-line">{e['line']}</td><td class="tag-line">{e['column']}</td><td>{e['message']}</td></tr>"""
        return rows

    total = len(lex_errors) + len(syn_errors)
    all_rows = make_rows(lex_errors, 'Léxico') + make_rows(syn_errors, 'Sintáctico')
    content = '<div class="no-errors">No se encontraron errores.</div>' if total == 0 else f"""<table><thead><tr><th>Tipo</th><th>Línea</th><th>Columna</th><th>Mensaje</th></tr></thead><tbody>{all_rows}</tbody></table>"""
    badge_class = "ok" if total == 0 else "badge"

    html = f"""<!DOCTYPE html>
<html lang="es"><head><meta charset="UTF-8"><title>PitCode - Errores</title>{HTML_STYLE}</head>
<body>
  <h1>PitCode — Reporte de Errores</h1>
  <div class="summary">Archivo: <strong>{source_file}</strong> | Léxicos: <strong>{len(lex_errors)}</strong> | Sintácticos: <strong>{len(syn_errors)}</strong></div>
  <h2>Errores <span class="badge {badge_class}">{total}</span></h2>
  {content}
  <footer>PitCode Compiler · Compiladores 2026 · Fase I</footer>
</body></html>"""
...
def main():
    ...
    if lex_errors:
        ast = None
        syn_errors = []
        print("        Análisis sintáctico omitido por errores léxicos")
    else:
    ...
    generate_token_report(token_list, lex_errors, base, os.path.join(reports_dir, 'reporte_tokens.html'))
    generate_symbol_table_report(symbol_table, base, os.path.join(reports_dir, 'reporte_simbolos.html'))
    generate_error_report(lex_errors, syn_errors, base, os.path.join(reports_dir, 'reporte_errores.html'))
