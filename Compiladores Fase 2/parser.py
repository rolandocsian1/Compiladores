...
def p_telemetry(p):
    ...
    p[0] = ('telemetry', p[3], p.lineno(3), p.lexpos(3))
...
def p_expr_binop(p):
    ...
    p[0] = ('binop', p[2], p[1], p[3], p.lineno(2), p.lexpos(2))

def p_expr_uminus(p):
    '''expression : GAP expression %prec UMINUS'''
    p[0] = ('uminus', p[2], p.lineno(1), p.lexpos(1))

def p_expr_not(p):
    '''expression : REVERSE_GRID expression
                  | REVERSE expression'''
    p[0] = ('not', p[2], p.lineno(1), p.lexpos(1))
...
def p_expr_assign(p):
    ...
    p[0] = ('assign', p[1], p[3], p.lineno(1), p.lexpos(1))
...
def p_expr_compound_assign(p):
    ...
    p[0] = ('compound_assign', p[2], p[1], p[3], p.lineno(1), p.lexpos(1))

def p_expr_increment(p):
    '''expression : ID FASTLAP'''
    p[0] = ('increment', p[1], p.lineno(1), p.lexpos(1))

def p_expr_decrement(p):
    '''expression : ID DEGRADATION'''
    p[0] = ('decrement', p[1], p.lineno(1), p.lexpos(1))
...
def p_expr_call_with_args(p):
    ...
    p[0] = ('call', p[1], p[3], p.lineno(1), p.lexpos(1))

def p_expr_call_no_args(p):
    '''expression : ID CORNER APEX'''
    p[0] = ('call', p[1], [], p.lineno(1), p.lexpos(1))
