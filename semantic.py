...
class SemanticAnalyzer:
    ...
    def _register_function(self, node):
        ...
        if name in self.functions:
            self._add_error(f"Función '{name}' ya fue declarada", line, 0)
        else:
            self.functions[name] = {
    ...
    def _visit_declare(self, node):
        ...
        self._declare_var(name, dtype, line, 0)
    ...
    def _visit_declare_const(self, node):
        dtype, name, expr = node[1], node[2], node[3]
        line = node[4] if len(node) > 4 else 0
        self._declare_var(name, dtype, line, 0, const=True)
        self.vars_declared += 1
        if expr:
            self._visit(expr)
    ...
    def _visit_assign(self, node):
        ...
        if not var:
            self._add_error(f"Variable '{name}' no declarada", 0, 0)
            return
        if var.get('const'):
            self._add_error(f"No se puede modificar la constante '{name}'", 0, 0)
            return
        var['used'] = True
        self.vars_used += 1
        expr_type = self._visit(expr)
    ...
    def _visit_increment(self, node):
        ...
        if internal_type not in ('entero', 'decimal'):
            self._add_error(f"No se puede incrementar '{name}' de tipo '{internal_type}'", 0, 0)
    ...
    def _visit_decrement(self, node):
        ...
        internal_type = self.TYPE_MAP.get(var['type'], var['type'])
        if internal_type not in ('entero', 'decimal'):
            self._add_error(f"No se puede decrementar '{name}' de tipo '{internal_type}'", 0, 0)
    ...
    def _visit_break(self, node):
        if self.loop_depth == 0:
            self._add_error("'box_box' (break) usado fuera de un ciclo", 0, 0)

    def _visit_continue(self, node):
        if self.loop_depth == 0:
            self._add_error("'drs' (continue) usado fuera de un ciclo", 0, 0)
    ...
    def _visit_telemetry(self, node):
        ...
        var = self._lookup_var(name)
        if not var:
            self._add_error(f"Variable '{name}' no declarada en telemetry", 0, 0)
        else:
            var['used'] = True
    ...
    def _visit_uminus(self, node):
        ...
        if t and t not in ('entero', 'decimal'):
            self._add_error(f"Negación no válida para tipo '{t}'", 0, 0)
        return t
    ...
    def _visit_call(self, node):
        ...
        if name not in self.functions:
            self._add_error(f"Función '{name}' no declarada", 0, 0)
            return None
